...
class ArgusDaemon:
    ...
    def _setup_jobs(self) -> None:
        ...
        for stream_name in self._iter_enabled_streams():
            ...
            if self.daemon_config.is_job_enabled(JOB_CRYPTO_DAILY):
    ...
    async def _run_crypto_daily(self, trigger_type: str = "scheduled", stream_name: Optional[str] = None) -> Optional[JobRunRecord]:
        """Run crypto daily update job."""
        return await self._run_job(
            job_id=JOB_CRYPTO_DAILY,
            job_func=self._run_crypto_daily,
            trigger_type=trigger_type,
            stream_name=stream_name,
        )

    async def _run_crypto_daily_sync(config: ArgusConfig) -> Any:
        """Run crypto daily update job (sync wrapper)."""
        from argus.orchestrator import OrchestratorOptions, RunOrchestrator, RunMode
    ...
    async def _run_crypto_daily_for_stream(self, stream_name: str) -> Optional[JobRunRecord]:
